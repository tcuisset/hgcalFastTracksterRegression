""" Training using v2 samples (skeleton) pions using the main trackster output by the single CLUE3D iteration """
import os
import torch
...
outputPath = "/grid_mnt/data_cms_upgrade/cuisset/ticlRegression/models/v5"
...
def doTrainVal(tag, model, optimizer, nepochs=50, loss=loss_mse_basic, resultsFromModel_fct=getResultsFromModel_basicLoss):
    input = AkSampleLoader.loadFromPickle("/grid_mnt/data_cms_upgrade/cuisset/ticlRegression/PionSamples_v2/fullData.pkl").selectMainTrackster()
    train_dataloader = makeDataLoader(makeTrainingSample(input, feature_version), batch_size=1000, num_workers=10)
    model.to(device)
